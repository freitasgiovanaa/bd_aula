...
select b.nome, sum(valor) as total_emprestado from emprestimo emp
inner join banco b on b.id_banco = emp.id_banco
group by b.nome

select e.id_emprestimo, e.numero_parcelas - count(p.id_emprestimo)
as parcelas_restantes
from emprestimo e 
inner join pagamentos p on e.id_emprestimo = p.id_emprestimo
group by (e.id_emprestimo)

select c.nome, e.valor, p.valor_pagamento, e.valor - p.valor_pagamento 
as valor_restante from emprestimo e
inner join cliente c on e.id_cliente = c.id_cliente
inner join pagamentos p on e.id_emprestimo = p.id_emprestimo
order by c.nome

select e.id_emprestimo, sum(p.valor_pagamento) as valor_pago,
e.valor - sum(p.valor_pagamento) as valor_restante, 
e.numero_parcelas as parcelas_restantes 
from emprestimo e
inner join pagamentos p on p.id_emprestimo = e.id_emprestimo
